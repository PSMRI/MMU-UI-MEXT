...
export class CovidVaccinationStatusComponent implements OnInit, DoCheck {
  ...
  applicableForVaccineStatusList: any = [
        ...
        beneficiaryDetails => {
          if (beneficiaryDetails) {
            ...
            if (this.beneficiaryAge < 12) {
              this.covidVaccineStatusForm.controls['ageGroup'].setValue(
                '<12 years'
              );
              this.doctorService.covidVaccineAgeGroup = '<12 years';
              this.covidVaccineStatusForm.controls[
                'isApplicableForVaccine'
              ].setValue('Not Applicable for Vaccination');
              ...
              this.covidVaccineStatusForm.controls[
                'isApplicableForVaccine'
              ].setValue('Applicable for Vaccination');
  ...
  setIsApplicable() {
    ...
    this.doctorService.enableCovidVaccinationButton = false;
    this.doctorService.covidVaccineAgeGroup =
      this.covidVaccineStatusForm.controls['ageGroup'].value;

    if (
      this.covidVaccineStatusForm.controls['ageGroup'].value === '<12 years'
    ) {
      this.covidVaccineStatusForm.controls['isApplicableForVaccine'].setValue(
        'Not Applicable for Vaccination'
      );
        ...
        (res: any) => {
          if (res.statusCode === 200) {
            if (res.data.covidVSID) {
              ...
              this.covidVaccineStatusForm.controls['vaccineStatus'].setValue(
                res.data.vaccineStatus
              );
              this.enableVaccineTypeAndDoseTaken();
              if (res.data.vaccineStatus === 'YES') {
                this.covidVaccineStatusForm.controls['vaccineTypes'].setValue(
                  res.data.covidVaccineTypeID
                );
                this.covidVaccineStatusForm.controls['doseTaken'].setValue(
                  res.data.doseTypeID
                );
              }
              this.enableSaveButton = true;
  ...
  enableSaveButtonInForm() {
    if (
      this.covidVaccineStatusForm.controls['vaccineTypes'].value !==
        undefined &&
      this.covidVaccineStatusForm.controls['vaccineTypes'].value !== null &&
      this.covidVaccineStatusForm.controls['doseTaken'].value !== undefined &&
      this.covidVaccineStatusForm.controls['doseTaken'].value !== null
    ) {
      this.enableSaveButton = false;
      this.doctorService.enableCovidVaccinationButton = true;
    } else {
